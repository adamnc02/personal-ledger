...
export type TransactionType =
  ...
  | 'credit_card_payment'
  // SUPERSEDED (backlog item a, savings overhaul) — no longer generated by
  // anything. Kept in the union purely so an already-persisted backup
  // containing old savings_contribution rows still type-checks/loads;
  // clearTransaction.ts's side effect for it is left in place for the
  // same reason. New savings activity uses the three types below instead.
  ...
  // convention as savings_contribution above). `direction`/`location`/
...
export interface Transaction {
  ...
  sourceType?:
    ...
    | 'savings_entry' // superseded — only ever seen on an already-persisted legacy row, see TransactionType
    | 'pension'
...
export interface Person {
  ...
  // SUPERSEDED (backlog item a, savings overhaul, 2026-09-02 session) —
  // savings is now its own top-level, personId-owned SavingsPot table
  // (below), architecturally identical to Pension/Loan, matching the
  // household-ownership decision this whole area already follows.
  // Nothing in the app creates new entries here any more — the field
  // stays only so an already-persisted backup containing real legacy
  // goals/plans still round-trips (lib/legacyBridge.ts, lib/savings.ts,
  // clearTransaction.ts's savings_contribution side effect all still
  // read/write it for exactly that reason). New code should never read
  // or write this field. Genuinely orphaned by this: Scenarios.tsx's
  // "target a savings goal" what-if action reads this array to populate
  // its picker — since nothing can add to it any more, that option will
  // simply never show for anyone who didn't already have a legacy goal
  // before this session. Flagging rather than fixing silently: item d
  // (what-if scenarios) is its own separate backlog entry and hasn't
  // been touched this session — whoever picks that item up next should
  // decide whether to re-point it at SavingsPot targets.
