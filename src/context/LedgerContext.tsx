...
// exists. This removes exactly the pending ones and nothing else.
//
...
export function sweepPendingForLoan(transactions: Transaction[], loan: Loan): Transaction[] {
...
// unlike loans above.
export function sweepPendingForCreditCard(transactions: Transaction[], cardId: string): Transaction[] {
  return transactions.filter((t) => t.status !== 'pending' || t.creditCardId !== cardId)
}
export function sweepPendingForSavingsPot(transactions: Transaction[], potId: string): Transaction[] {
  return transactions.filter((t) => t.status !== 'pending' || t.savingsPotId !== potId)
}
...
export function sweepPendingForPot(transactions: Transaction[], potId: string): Transaction[] {
  return transactions.filter((t) => t.status !== 'pending' || t.potId !== potId)
}
...
export function sweepPendingForSource(transactions: Transaction[], sourceType: NonNullable<Transaction['sourceType']>, sourceId: string): Transaction[] {
  return transactions.filter((t) => t.status !== 'pending' || t.sourceType !== sourceType || t.sourceId !== sourceId)
}
