# Personal Ledger — Technical Documentation

Implementation-level reference for `personal-ledger`. README.md stays user-facing: what the app
does, screen by screen. This document is how it is built, and — more importantly — **why it is
built that way**, because most of the non-obvious decisions here were paid for with real bugs
against real data.

> Companion documents, outside this repo:
> `Downloads/App Development & Bug Tracking/shared-finance-ledger/APP-KNOWLEDGE.md` (the
> invariants register), `DATA-MODEL-REVIEW-2026-09-15.md` (what the data actually looks like),
> `DIVERGENCE.md` (what may differ between the two live apps), and the dated PROMPT/UAT files in
> `Downloads/App Development & Bug Tracking/personal-ledger/`.
...
for date arithmetic, `lucide-react` for icons, `nanoid` for ids. No backend, no server, no auth.
...
│      │ store.load()                 ▼                      │
│  lib/store/LedgerStore.ts ── localStorageLedgerStore       │
│                                  └─ localStorage           │
│                                     'ledger:app-data-v2:v1'│
│                                                            │
...
src/
  ...
  lib/
    store/
      ...
      localStorageLedgerStore.ts The offline implementation
  ...
  components/       ~33 shared components (see §7–§8)
scripts/
  verify-*.ts       134 executable checks
...
> 🚨 This app has real, unbacked-up data under that key. `verify-ledger-store.ts` proves the store
> reads and writes it byte-for-byte as before, and snapshots the context's 76-member public API so
> a mutation cannot be added or removed silently.

**Why the interface exists at all:** so `shared-finance-ledger` can pass `powerSyncLedgerStore`
from its `App.tsx` and `LedgerContext.tsx` stays byte-identical in both repos. **If this file ever
needs to differ between the apps, the interface is leaking — reconsider it rather than forking
the file.**
...
migrated data on startup. That is deliberate, and the sync store's first-sync gate exists partly
because of it.
...
| `HeaderAccessory` | **A deliberately empty slot.** Renders nothing unless an app fills it — which is how the sync app puts an Account button in the Wallet header while `Salary.tsx` stays identical in both repos |
...
cycle statement builds its sections from the SAME list the Home page swipes through — see §36. A
...
🚨 **`?raw` inlines the template at build time — it is never fetched.** `personal-ledger` is
permanently offline by design, and a fetched template would fail silently on the app that can least
afford it. Anything running outside Vite (every verify script) reads the file from disk instead,
